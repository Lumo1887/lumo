export type ModuleStatus = "active" | "coming-soon";

export interface ModuleDefinition {
  slug: string;
  faculty: string;
  title: string;
  subtitle: string;
  status: ModuleStatus;
  priceCent: number;
  compareAtCent: number; // Vergleichspreis (klassischer Vorbereitungskurs)
  chapters: string[];
  questionCount: number;
  // Für die dreistufige Auswahl auf dem Dashboard (Studiengang → Fach → Nummer):
  studiengang: string; // z. B. "Wirtschaftsingenieurwesen"
  subject: string; // Fach-Gruppe, z. B. "Statistik", "VWL"
  number: string; // Nummer/Variante innerhalb des Fachs, z. B. "I", "2" — leer, wenn es (noch) nur eine Variante gibt
  popular?: boolean; // Wird auf dem Dashboard standardmäßig (ohne Filter) angezeigt
  exampleQuestion?: string; // Beispielfrage, die im Chatbot als Vorschlag angezeigt wird
}

// Studiengänge, die auf dem Dashboard zur Auswahl stehen. Aktuell ist nur
// Wirtschaftsingenieurwesen mit echten Modulen hinterlegt — die anderen
// werden ausgegraut mit "bald verfügbar" angezeigt, damit sichtbar ist,
// dass Lumo wächst, ohne schon Inhalte dafür zu versprechen.
export const STUDIENGAENGE: { name: string; active: boolean }[] = [
  { name: "Wirtschaftsingenieurwesen", active: true },
  { name: "Maschinenbau", active: false },
  { name: "Elektrotechnik und Informationstechnik", active: false },
  { name: "Informatik", active: false },
];

export const modules: ModuleDefinition[] = [
  {
    slug: "statistik-1",
    faculty: "WiWi-Fakultät",
    title: "Statistik I",
    subtitle: "Deskriptive Statistik, Wahrscheinlichkeitsrechnung, Zufallsvariablen & Verteilungsmodelle",
    status: "active",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [
      "Wahrscheinlichkeitsrechnung",
      "Zufallsvariablen",
      "Verteilungsmodelle",
      "Deskriptive Statistik",
    ],
    questionCount: 45,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Statistik",
    number: "I",
    popular: true,
    exampleQuestion: "Erklär mir den Satz von Bayes an einem Beispiel.",
  },
  {
    slug: "vwl-2",
    faculty: "WiWi-Fakultät",
    title: "VWL 2: Makroökonomie",
    subtitle: "BIP, Wachstum, Arbeitslosigkeit, Geld & Inflation, Konjunktur (IS-LM), offene Volkswirtschaften & Gesamtangebot",
    status: "active",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [
      "Methodik der Makroökonomik",
      "Das Bruttoinlandsprodukt",
      "Geld und Inflation",
      "Bankensystem und Geldschöpfung",
      "Arbeitslosigkeit",
      "Wirtschaftswachstum I: Das Solow-Modell",
      "Wirtschaftswachstum II: Bevölkerung, Fortschritt und Konvergenz",
      "Exkurs: Automatisierung und Arbeitsmarkt",
      "Exkurs: Klima und Wachstum",
      "Konjunktur: Das IS-LM-Modell",
      "Gesamtwirtschaftliches Angebot: AS-AD-Modell und Phillipskurve",
      "Offene Volkswirtschaften: Mundell-Fleming und Wechselkurse",
      "Exkurs: Zölle und Handelspolitik",
    ],
    questionCount: 53,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "VWL",
    number: "2",
    popular: true,
    exampleQuestion: "Erklär mir das IS-LM-Modell an einem Beispiel.",
  },
  {
    slug: "statistik-2",
    faculty: "WiWi-Fakultät",
    title: "Statistik II",
    subtitle: "Schätzen, Testen, Regression",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Statistik",
    number: "II",
  },
  {
    slug: "vwl-1",
    faculty: "WiWi-Fakultät",
    title: "VWL 1: Mikroökonomie",
    subtitle: "Haushalte, Unternehmen, Märkte & Wettbewerb",
    status: "active",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [
      "Markt und Budgetmenge",
      "Präferenzen und Nutzen",
      "Optimale Entscheidung und Nachfrage",
      "Konsumentenrente, Marktnachfrage und Marktgleichgewicht",
      "Bekundete Präferenzen und Slutsky-Zerlegung",
      "Anfangsausstattung, Nettohandel und Arbeitsangebot",
      "Technologie und Gewinnmaximierung der Unternehmung",
      "Kostenminimierung und Kostenkurven",
      "Angebot der Unternehmung und Marktangebot der Branche",
      "Monopol",
      "Spieltheorie",
      "Oligopoltheorie",
      "Tausch, Wohlfahrtstheorie und intertemporale Entscheidung",
    ],
    questionCount: 62,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "VWL",
    number: "1",
    exampleQuestion: "Erklär mir das Gewinnmaximierungsproblem des Monopolisten an einem Beispiel.",
  },
  {
    slug: "mathematik-1-semester",
    faculty: "WiWi-Fakultät",
    title: "Mathematik 1: Semesterklausur",
    subtitle: "Aussagenlogik, Mengenlehre, Relationen, Abbildungen, Folgen, Reihen & Potenzreihen — Stoff der Semesterklausur (1. Prüfungsteil)",
    status: "active",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [
      "Aussagenlogik",
      "Mengenlehre",
      "Exkurs: Algebraische Strukturen",
      "Kombinatorik und das Laplace-Modell",
      "Relationen",
      "Vollständige Induktion und die natürlichen Zahlen",
      "Abbildungen",
      "Die reellen Zahlen",
      "Folgen",
      "Teilfolgen, Häufungspunkte und Cauchy-Folgen",
      "Reihen",
      "Potenzreihen und spezielle Funktionen",
    ],
    questionCount: 77,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Mathematik",
    number: "1 – Semesterklausur",
    popular: true,
    exampleQuestion: "Erklär mir die vollständige Induktion an einem Beispiel.",
  },
  {
    slug: "mathematik-1-abschluss",
    faculty: "WiWi-Fakultät",
    title: "Mathematik 1: Abschlussklausur",
    subtitle: "Grundlagen der Analysis und linearen Algebra — Stoff der Abschlussklausur (2. Prüfungsteil)",
    status: "active",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [
      "Lineare Gleichungssysteme und der Gauß-Algorithmus",
      "Vektorräume und Untervektorräume",
      "Funktionsgrenzwerte und Stetigkeit",
      "Exkurs: Exponential- und Logarithmusfunktionen",
      "Lineare Abbildungen und Matrizen",
      "Differenzierbarkeit und Ableitungsregeln",
      "Extremwerte, Mittelwertsatz und Kurvendiskussion",
      "Exkurs: Die Regel von de l'Hospital und der Satz von Taylor",
      "Skalarprodukte und Orthogonalität",
      "Integralrechnung",
    ],
    questionCount: 81,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Mathematik",
    number: "1 – Abschlussklausur",
    exampleQuestion: "Erklär mir den Hauptsatz der Differential- und Integralrechnung an einem Beispiel.",
  },
  {
    slug: "mathematik-2-semester",
    faculty: "WiWi-Fakultät",
    title: "Mathematik 2: Semesterklausur",
    subtitle: "Fortführung Analysis & lineare Algebra — Stoff der Semesterklausur (1. Prüfungsteil)",
    status: "active",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [
      "Komplexe Zahlen",
      "Folgen und Häufungspunkte im Rⁿ",
      "Determinanten",
      "Basisdarstellung und Basiswechsel",
      "Eigenwerte und Eigenräume",
      "Definite Matrizen",
      "Normen, Skalarprodukte und Orthogonalität",
      "Stetigkeit mehrdimensionaler Funktionen",
      "Gradient, Richtungsableitung und Tangentialebene",
    ],
    questionCount: 72,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Mathematik",
    number: "2 – Semesterklausur",
    exampleQuestion: "Erklär mir, wie man die dritten Wurzeln einer komplexen Zahl berechnet.",
  },
  {
    slug: "mathematik-2-abschluss",
    faculty: "WiWi-Fakultät",
    title: "Mathematik 2: Abschlussklausur",
    subtitle: "Mehrdimensionale Funktionen & Fortführung — Stoff der Abschlussklausur (2. Prüfungsteil)",
    status: "active",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [
      "Partielle Ableitungen und der Gradient",
      "Richtungsableitungen und der Gradient als steilster Anstieg",
      "Totale Differenzierbarkeit und die Tangentialebene",
      "Kettenregel, Jacobi-Matrix und vektorwertige Funktionen",
      "Höhere Ableitungen, der Satz von Schwarz und Taylorpolynome",
      "Lokale Extrema, die Hessematrix und Sattelpunkte",
      "Jordan-Inhalt und das Riemann-Integral im Rⁿ",
      "Normalbereiche, das Prinzip von Cavalieri und der Transformationssatz",
    ],
    questionCount: 70,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Mathematik",
    number: "2 – Abschlussklausur",
    exampleQuestion: "Erklär mir die Hessematrix und Sattelpunkte an einem Beispiel.",
  },
  {
    slug: "mathematik-3",
    faculty: "WiWi-Fakultät",
    title: "Mathematik 3",
    subtitle: "Vertiefende Mathematik für Wirtschaftsingenieure",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Mathematik",
    number: "3",
  },
  {
    slug: "bwl-1",
    faculty: "WiWi-Fakultät",
    title: "Management und Marketing",
    subtitle: "Grundlagen des Managements und Marketings",
    status: "active",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [
      "Grundlagen: Was Unternehmer und Unternehmen leisten",
      "Gründerpersonen, Motive und unternehmerische Gelegenheiten",
      "Entscheiden unter Risiko und Unsicherheit",
      "Geschäftsmodelle systematisch entwickeln und planen",
      "Vom Prototyp zum Pivot: Lean Startup und Design Thinking",
      "Organisation des Unternehmens",
      "Strategische Analyse und strategische Optionen",
      "Corporate Governance und Personalführung",
      "Kundenverhalten verstehen: von Daten zu Erkenntnissen",
      "Produktgestaltung, Preispolitik und Kommunikationspolitik",
      "Erfolg beim Kunden messen: CLV und Customer Equity",
    ],
    questionCount: 91,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "BWL",
    number: "1",
    exampleQuestion: "Wie berechne ich den Customer Lifetime Value bei bekannter Wiederkaufrate?",
  },
  {
    slug: "bwl-2",
    faculty: "WiWi-Fakultät",
    title: "Finanzierung und Rechnungswesen",
    subtitle: "Grundlagen der Finanzbuchhaltung und Unternehmensfinanzierung",
    status: "active",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [
      "Zeitwert des Geldes: Barwert und Endwert",
      "Rentenformeln: Ewige und endliche Renten",
      "Investitionsrechnung I: Kapitalwert und Amortisationsdauer",
      "Investitionsrechnung II: Interner Zinssatz und Kapitalwertrate",
      "Bewertung von Anleihen",
      "Eigenkapitalfinanzierung: Aktien und Ausschüttungspolitik",
      "Portfoliotheorie und CAPM",
      "Grundsätze der Rechnungslegung",
      "Grundsätze ordnungsmäßiger Buchführung und das Buchungssystem",
      "Grundlagen der Kosten- und Leistungsrechnung",
      "Bewertungswahlrechte im externen Rechnungswesen",
      "Der Betriebsabrechnungsbogen und Kalkulationsverfahren",
      "Deckungsbeitragsrechnung",
    ],
    questionCount: 69,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "BWL",
    number: "2",
    exampleQuestion: "Wie berechne ich den Kapitalwert einer Investition bei bekanntem Zahlungsstrom und Zinssatz?",
  },
  {
    slug: "bwl-3",
    faculty: "WiWi-Fakultät",
    title: "Produktion, Logistik und Wirtschaftsinformatik",
    subtitle: "Produktionswirtschaft, Logistik und Wirtschaftsinformatik",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "BWL",
    number: "3",
  },
  {
    slug: "informatik-1",
    faculty: "WiWi-Fakultät",
    title: "Programmieren 1: Java",
    subtitle: "Java-Grundlagen und objektorientierte Programmierung",
    status: "active",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [
      "Kontrollstrukturen: Verzweigungen und Schleifen",
      "Einfache Datentypen, Operatoren und Typumwandlung",
      "Methoden: Aufbau, Parameter und Gültigkeitsbereiche",
      "Rekursive Methoden",
      "Referenzdatentypen: Arrays und Strings",
      "Einführung in die Objektorientierung",
      "Klassen im Detail: Konstruktoren und Kapselung",
      "Vererbung",
      "Polymorphismus und besondere Klassen im JDK",
      "Ausnahmebehandlung",
      "Listen, Bäume und rekursive Datenstrukturen",
    ],
    questionCount: 87,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Informatik",
    number: "1",
    exampleQuestion: "Erklär mir Polymorphismus und dynamisches Binden an einem Beispiel.",
  },
  {
    slug: "informatik-2",
    faculty: "WiWi-Fakultät",
    title: "Grundlagen der Informatik",
    subtitle: "Logik, Algorithmen, Datenstrukturen, Sortieren, Komplexität & UML",
    status: "active",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [
      "Aussagenlogik",
      "Prädikatenlogik",
      "Boolesche Algebra",
      "Algorithmen: Grundbegriffe und Eigenschaften",
      "Entwurfsprinzipien und Testen von Algorithmen",
      "Datenstrukturen I: Abstrakte Datentypen, Listen, Keller und Schlangen",
      "Datenstrukturen II: Graphen, Bäume und binäre Suchbäume",
      "Komplexität von Algorithmen",
      "Sortierverfahren",
      "Datenstrukturen III: Kürzeste Wege und minimale Spannbäume",
      "UML: Anwendungsfall-, Klassen- und Objektdiagramme",
      "UML: Verhaltensdiagramme",
    ],
    questionCount: 86,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Informatik",
    number: "2",
    exampleQuestion: "Erklär mir binäre Suchbäume an einem Beispiel.",
  },
  {
    slug: "or-1",
    faculty: "WiWi-Fakultät",
    title: "Einführung in das Operations Research",
    subtitle: "Lineare Optimierung, Netzplantechnik & Entscheidungsmodelle",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Operations Research",
    number: "",
  },
  {
    slug: "ing-1",
    faculty: "WiWi-Fakultät",
    title: "Werkstoffkunde I",
    subtitle: "Bindungslehre, Kristallaufbau, mechanisches Verhalten und Zustandsdiagramme",
    status: "active",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [
      "Werkstoffgruppen und Bindungsarten",
      "Atomarer Aufbau und Periodensystem",
      "Elastisches Verhalten: Spannung, Dehnung, Hookesches Gesetz",
      "Kristallgitter: Bravais-Systeme und dichteste Packungen",
      "Röntgenographische Strukturanalyse",
      "Legierungsstrukturen und Mischkristalle",
      "Ionen-, kovalente und Polymerstrukturen",
      "Kristallbaufehler",
      "Plastische Verformung, Verfestigung und Bruch",
      "Elektrische und thermische Eigenschaften",
      "Thermodynamik von Phasenübergängen",
      "Zweistoffsysteme und Zustandsdiagramme",
      "Das Eisen-Kohlenstoff-System",
    ],
    questionCount: 47,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Ingenieurwissenschaften",
    number: "1",
    exampleQuestion: "Erklär mir die Hebelregel im Zustandsdiagramm an einem Beispiel.",
  },
  {
    slug: "ing-2",
    faculty: "WiWi-Fakultät",
    title: "Technische Mechanik",
    subtitle: "Statik und Festigkeitslehre",
    status: "active",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [
      "Kräfte, Axiome und der starre Körper",
      "Kraftgruppen, Hebelgesetz und das Moment einer Kraft",
      "Gleichgewicht, Lagerarten und statische Bestimmtheit",
      "Haftreibung, Seilreibung und der Kraftkegel",
      "Fachwerke: Knotenschnitt- und Rittersches Verfahren",
      "Schnittgrößen am geraden Balken",
      "Spannungen, Dehnungen und das Hookesche Gesetz",
      "Flächenträgheitsmoment und der Satz von Steiner",
      "Gerade Biegung und die Biegelinie",
      "Mehrteilige Tragwerke und der Gelenkbalken",
      "Exkurs: Mehrachsiger Spannungszustand und der Mohrsche Spannungskreis",
      "Exkurs: Knicken von Stäben nach Euler",
    ],
    questionCount: 61,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Ingenieurwissenschaften",
    number: "2",
    exampleQuestion: "Erklär mir den Satz von Steiner an einem Beispiel.",
  },
  {
    slug: "ing-3",
    faculty: "WiWi-Fakultät",
    title: "Elektrotechnik",
    subtitle: "Grundlagen der Elektrotechnik",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Ingenieurwissenschaften",
    number: "3",
  },
  {
    slug: "ing-4",
    faculty: "WiWi-Fakultät",
    title: "Weiterführende ingenieurwissenschaftliche Grundlagen",
    subtitle: "Vertiefende ingenieurwissenschaftliche Inhalte",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Wirtschaftsingenieurwesen",
    subject: "Ingenieurwissenschaften",
    number: "4",
  },
  // ==================== Maschinenbau (noch nicht aktiv, siehe STUDIENGAENGE) ====================
  {
    slug: "mb-mathe",
    faculty: "Maschinenbau-Fakultät",
    title: "Höhere Mathematik",
    subtitle: "Analysis und lineare Algebra für Maschinenbau-Ingenieure",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Maschinenbau",
    subject: "Mathematik",
    number: "",
  },
  {
    slug: "mb-mechanik",
    faculty: "Maschinenbau-Fakultät",
    title: "Technische Mechanik",
    subtitle: "Statik, Festigkeitslehre und Dynamik",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Maschinenbau",
    subject: "Mechanik",
    number: "",
  },
  {
    slug: "mb-werkstoffkunde",
    faculty: "Maschinenbau-Fakultät",
    title: "Werkstoffkunde",
    subtitle: "Werkstoffe und ihre Eigenschaften für den Maschinenbau",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Maschinenbau",
    subject: "Werkstoffe",
    number: "",
  },
  {
    slug: "mb-konstruktionslehre",
    faculty: "Maschinenbau-Fakultät",
    title: "Maschinenkonstruktionslehre",
    subtitle: "Grundlagen der Maschinenelemente und Konstruktion",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Maschinenbau",
    subject: "Konstruktion",
    number: "",
  },
  {
    slug: "mb-fertigungstechnik",
    faculty: "Maschinenbau-Fakultät",
    title: "Fertigungstechnik",
    subtitle: "Fertigungsverfahren im Maschinenbau",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Maschinenbau",
    subject: "Fertigungstechnik",
    number: "",
  },
  {
    slug: "mb-thermodynamik",
    faculty: "Maschinenbau-Fakultät",
    title: "Technische Thermodynamik",
    subtitle: "Grundlagen der Thermodynamik für Ingenieure",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Maschinenbau",
    subject: "Thermodynamik",
    number: "",
  },
  {
    slug: "mb-stroemungslehre",
    faculty: "Maschinenbau-Fakultät",
    title: "Strömungslehre",
    subtitle: "Grundlagen der Fluidmechanik",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Maschinenbau",
    subject: "Strömungslehre",
    number: "",
  },
  {
    slug: "mb-elektrotechnik",
    faculty: "Maschinenbau-Fakultät",
    title: "Elektrotechnik und Mechatronik",
    subtitle: "Grundlagen der Elektrotechnik und Mechatronik für den Maschinenbau",
    status: "coming-soon",
    priceCent: 499,
    compareAtCent: 1500,
    chapters: [],
    questionCount: 0,
    studiengang: "Maschinenbau",
    subject: "Elektrotechnik",
    number: "",
  },
];

export function getModule(slug: string): ModuleDefinition | undefined {
  return modules.find((m) => m.slug === slug);
}

export function formatEuro(cent: number): string {
  return (cent / 100).toLocaleString("de-DE", {
    style: "currency",
    currency: "EUR",
  });
}
